...
function bySelection() {
  var ss = SpreadsheetApp.getSelection();
  var activeRange = ss.getActiveRange();
  var numRows = activeRange.getNumRows();
  var colRows = activeRange.getNumColumns();
  var dataArray = activeRange.getValues()
  activeRange.setValues(convert(dataArray));
}

function convert(data)
{
  for (var xIndex in data)
  {
    var xData = data[xIndex];
    for(var yIndex in xData)
    {
      data[xIndex][yIndex] = zg2uni(data[xIndex][yIndex]); 
    }
  }
  return data;
}
...
function onOpen(e) {
  SpreadsheetApp.getUi()
      .createMenu('ZG2Uni')
      .addItem('By Selection', 'bySelection')
      .addToUi();
}
